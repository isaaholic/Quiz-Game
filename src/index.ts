...
function FindIntELementFromArray(arr: number[], element: number): boolean {
...
for (var i = 0; i < questions.length; i++) {
  ...
  for (var j = 1; j < 4; j++) {
    while (true) {
      ...
      if (FindIntELementFromArray(answerNum, randNum)) continue;
      else {
        answerNum[k++] = randNum;
        break;
      }
  ...
  while (askQuestion) {
    ...
    for (var f = 0; f < answerNum.length; f++) {
      console.log(
        `${String.fromCharCode(f + 65)} ${questions[i][answerNum[f]]}`
      );
    }
    let key;
    switch ((key = inp())) {
    ...
    askQuestion = false;
    console.clear();
    if (questions[i][answerNum[choice]] == questions[i][1]) point += 10;
    else point -= 10;
    if (i != 9) {
